    ...
    def process_task(self, task_id: str) -> Dict[str, Any]:
        ...
        if state == "DISCOVERED":
            ...
            if not is_valid:
                ...
                self.db.update_task_state(task_id, "CANCELLED", error_reason=error_msg)
                self.logger.log_event("OPPORTUNITY_REJECTED", {
                    "task_id": task_id,
            ...
            self.logger.log_event("TASK_PLANNED", {
                "task_id": task_id,
                "opportunity_id": opp.id,
        ...
        if state == "PLANNED":
            ...
            if not budget_passed:
                ...
                self.db.update_task_state(task_id, "CANCELLED", error_reason=error_msg)
                self.logger.log_event("BUDGET_BLOCKED", {
                    "task_id": task_id,
                    "opportunity_id": opp.id,
            ...
            task["state"] = "READY"
            self.db.save_task(task)
            self.logger.log_event("TASK_READY", {
                "task_id": task_id,
                "opportunity_id": opp.id,
                "repo": repo
            })
        ...
        # ---------------------------------------------------------------------
        # 3. WORKER EXECUTION with Idempotency Guard (READY / EXECUTING -> REVIEWED)
        # ---------------------------------------------------------------------
        if state in ("READY", "EXECUTING") or (state == "REVIEWED" and task.get("review_result") is None):
            task_spec_data = task["task_spec"]
            task_spec = TaskSpec(**task_spec_data)

            if state in ("READY", "EXECUTING"):
                # Mark EXECUTING state in DB
                self.db.update_task_state(task_id, "EXECUTING")
                self.logger.log_event("TASK_EXECUTING", {
                    "task_id": task_id,
                    "opportunity_id": opp.id,
                    "repo": repo
                })
            ...
            if cached_worker_res:
                ...
                if worker_result.status == "FAILED":
                    ...
                    self.db.update_task_state(task_id, "FAILED", error_reason=worker_result.error_reason)
                    self.logger.log_event("WORKER_FAILED", {
                        "task_id": task_id,
                        "opportunity_id": opp.id,
                ...
                self.logger.log_event("WORKER_EXECUTED", {
                    "task_id": task_id,
                    "opportunity_id": opp.id,
            ...
            if cached_review_res:
                ...
                self.logger.log_event("REVIEWER_IDEMPOTENCY_HIT", {
                    "task_id": task_id,
                    "opportunity_id": opp.id
                })
            else:
                ...
                # Record actual spend into single-source-of-truth SQLite budget table
                task_cost = round(worker_result.actual_cost + review_result.review_cost, 6)
                self.budget_guard.record_spend(task_cost, opportunity_id=opp.id)

            task["review_result"] = review_result.to_dict()
            task["state"] = "REVIEWED"
            self.db.save_task(task)
            self.logger.log_event("REVIEW_COMPLETED", {
                "task_id": task_id,
                "opportunity_id": opp.id,
        ...
        # ---------------------------------------------------------------------
        # 5. APPROVAL GATE & COMPLETION (REVIEWED / WAITING_APPROVAL -> COMPLETED)
        # ---------------------------------------------------------------------
        if state in ("REVIEWED", "WAITING_APPROVAL"):
            ...
            self.db.update_task_state(task_id, "WAITING_APPROVAL")
            self.logger.log_event("TASK_WAITING_APPROVAL", {
                "task_id": task_id,
                "opportunity_id": opp.id
            })
            approval_decision = self.approval_gate.request_approval(opp, task_spec, worker_result, review_result)

            self.logger.log_event("APPROVAL_DECISION", {
                "task_id": task_id,
                "opportunity_id": opp.id,
            ...
            if approval_decision.approved and review_result.passed:
                self.db.update_task_state(task_id, "APPROVED")
                self.logger.log_event("TASK_APPROVED", {"task_id": task_id, "opportunity_id": opp.id})
                self.db.update_task_state(task_id, "DELIVERED")
                self.logger.log_event("TASK_DELIVERED", {"task_id": task_id, "opportunity_id": opp.id})
                self.db.update_task_state(task_id, "COMPLETED")
                ...
                self.db.update_task_state(task_id, "CANCELLED", error_reason=reason)
                self.logger.log_event("TASK_BLOCKED", {
                    "task_id": task_id,
                    "opportunity_id": opp.id,
                    "reason": reason
                })
